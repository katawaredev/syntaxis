...
const destination = resolve(outputDir, "browser-git.bundle.js");
const stamp = resolve(outputDir, "browser-git.bundle.stamp");
const manifest = resolve(root, "package.json");
const lockfile = resolve(root, "bun.lock");
const script = fileURLToPath(import.meta.url);
const cacheKey = createHash("sha256")
  .update("syntaxis-browser-git-v1\0")
...
if (
  ...
  console.log(`Browser Git bundle is current (${cacheKey.slice(0, 12)})`);
  process.exit(0);
}
...
console.log(`Built browser Git bundle (${cacheKey.slice(0, 12)})`);
