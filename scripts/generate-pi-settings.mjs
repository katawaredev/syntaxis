...
const declarations = readFileSync(
	resolve(packageRoot, "dist/core/settings-manager.d.ts"),
	"utf8",
);
const documentation = readFileSync(
	resolve(packageRoot, "docs/settings.md"),
	"utf8",
);
...
const exposed = [
	...
	[
		...
		"select:off|minimal|low|medium|high|xhigh|max",
	],
...
for (const line of documentation.split("\n")) {
	...
	let defaultValue =
		rawDefault.trim() === "-"
			? ""
			: rawDefault.trim().replace(/^`(.*)`$/, "$1");
	if (type.trim() === "string")
		defaultValue = defaultValue.replace(/^"(.*)"$/, "$1");
	if (type.trim() === "number" && !/^\d+$/.test(defaultValue))
		defaultValue = "";
	documented.set(path, {
		type: type.trim(),
		defaultValue,
...
const settings = exposed.map(([section, path, control]) => {
	...
	if (!metadata)
		throw new Error(
			`Pi ${manifest.version} docs no longer describe setting ${path}`,
		);
...
for (const [, path] of settings) {
	...
	if (!new RegExp(`\\n\\s*${topLevel}\\?:`).test(declarations)) {
		throw new Error(
			`Pi ${manifest.version} no longer declares setting ${path}`,
		);
	}
	const setter = setters.get(path);
	if (!setter || !new RegExp(`\\n\\s*${setter}\\(`).test(declarations)) {
		throw new Error(
			`Pi ${manifest.version} no longer exposes a supported setter for ${path}`,
		);
	}
}
...
if (existsSync(outputPath)) {
	...
	if (header.includes(`pi-settings-cache-digest: ${displayedCacheDigest}`)) {
		process.stdout.write(
			`Pi settings metadata is current (${cacheKey.slice(0, 12)})\n`,
		);
		process.exit(0);
	}
}
...
const rows = settings.map(
	([section, path, label, description, control, defaultValue]) => {
		let kind;
		if (control === "toggle") kind = "PiSettingKind::Toggle";
			...
			kind = `PiSettingKind::Select(&[${control.slice(7).split("|").map(rust).join(", ")}])`;
