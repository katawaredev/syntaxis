...
- **Editor boundary:** `assets/code-editor/bridge-source.js` and
  ...
  asset fetched when the editor mounts; language-server code is loaded separately
...
Recent Projects and editor readiness are explicit parts of the workload. The
benchmark uses the pinned `puppeteer-core` package with the machine's Chromium,
records the stable selectors in every result, and measures navigation start to
the Recent Projects section becoming non-loading. It then measures navigation
start to the fixture file's mounted, editable CodeMirror textbox. The standalone
`bun run autoresearch:recent-projects` command is useful while validating both
selector rules against an already-running benchmark server.

The release server uses a dedicated ignored data directory and a deterministic
registry generated by `autoresearch/provision.mjs`. The fixture is exposed
through the normal workspace registry and API; product code contains no fixture
or benchmark special cases. Lighthouse and browser measurements authenticate
through the normal bearer-token path with a local evaluator-only token.
