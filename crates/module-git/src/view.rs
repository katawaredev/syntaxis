...
pub fn GitView(
    ...
    on_activate_worktree: EventHandler<syntaxis_git::WorktreeInfo>,
) -> Element {
    let ports = use_context::<GitPorts>();
    let files_ports = use_context::<syntaxis_module_files::FilesPorts>();
    let can_branch = ports.branches().is_some();
    ...
    let initialize_workspace = workspace.clone();
    let initialize_mutations = ports.repository().cloned();
    let on_initialize = move |_| {
        let workspace = initialize_workspace.clone();
        let mutations = initialize_mutations.clone();
        pending.set(true);
    ...
    let on_compare = compare_handler(
        workspace.clone(),
        ports.clone(),
        pending,
        operation_error,
    ...
    let commit_mutations = ports.repository().cloned();
    let on_commit = move |request: CommitRequest| {
        let workspace = commit_workspace.clone();
        let mutations = commit_mutations.clone();
        ...
        spawn(async move {
            let Some(mutations) = mutations else {
                pending.set(false);
                operation_error.set(Some("Committing is unavailable in this runtime.".into()));
                return;
            };
    ...
    let on_history_action = EventHandler::new(move |(action, oid): (HistoryAction, String)| {
        ...
        match action {
            ...
            HistoryAction::CopyHash => copy_commit_hash(oid, files_ports.clone(), toast),
        }
    });

    let history_workspace = workspace.clone();
    let history_read = ports.history().cloned();
    let on_load_more_history = EventHandler::new(move |offset: usize| {
        ...
        spawn(async move {
            ...
            match result {
                ...
                Err(error) => history_page_error.set(Some(error.to_string())),
            }
        });
    });

    let signing_workspace = workspace.clone();
    let signing_mutations = ports.repository().cloned();
    let on_signing_retry = move |passphrase: String| {
        ...
        let workspace = signing_workspace.clone();
        let mutations = signing_mutations.clone();
        pending.set(true);
        operation_error.set(None);
        spawn(async move {
            let Some(mutations) = mutations else {
                    ...
                    PanelHeader { kind: PanelHeaderKind::Repository,
                                    ...
                                    DropdownMenu {
                                        ...
                                        div { class: "relative",
                                            ...
                                            MenuContent { class: "left-0 w-46",
                                                if can_merge { DropdownMenuItem::<GitDialog> {
                        ...
                        if !rebase_active {
                            div { class: "git-toolbar flex shrink-0 items-center gap-1",
                                if repository.conflict_count() > 0 && can_merge {
                                    button {
