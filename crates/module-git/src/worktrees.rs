...
pub(super) fn BranchWorktreeMenu(
    ...
    let files_session = use_context::<FilesUiState>();
    ...
    let worktree_port = ports.worktrees().cloned();
    let worktrees = use_resource(move || {
        let workspace = list_workspace.clone();
        let worktrees = worktree_port.clone();
    ...
    let worktree_list = worktrees().and_then(Result::ok).unwrap_or_default();
    let current_workspace_id = Some(workspace.id.0.clone());
        ...
        DropdownMenu {
            ...
            div { class: "relative min-w-0 @max-[520px]:flex-1",
                ...
                MenuContent { class: "left-0 w-76",
                                ...
                                div { class: "rounded-md",
                                    ...
                                    if options_expanded {
                                        div { class: "mx-1 mb-1 grid gap-0.5 border-l border-border pl-2",
                                            ...
                                            if ports.worktrees().is_some() && !row.branch.remote && attached.is_none() {
                                                button {
                                                    class: "min-h-7 rounded-sm px-2 text-left text-[10px] text-muted-foreground hover:bg-accent hover:text-foreground",
                        ...
                        for worktree in detached_worktrees.clone() {
                            div { class: "flex min-w-0 items-center gap-1 rounded-md",
                                button {
                                    ...
                                    onclick: {
                                        let worktree = worktree.clone();
                                        move |_| activate_worktree(worktree.clone())
                                    },
                    ...
                    if ports.worktrees().is_some() { hr {} }
                    if ports.worktrees().is_some() { div { title: create_worktree_tooltip,
                        button {
                            ...
                            disabled: busy || files_dirty || ports.worktrees().is_none() || !repository_has_commits,
                            onclick: {
                        ...
                        Button {
                            ...
                            onclick: move |_| {
                                ...
                                spawn(async move {
                                    ...
                                    match worktrees.create(&workspace, request).await {
                                        Ok(worktree) => {
                                            files_session.reset();
                                            create_mode.set(None);
                                            menu_open.set(false);
                        ...
                        Button {
                            ...
                            onclick: move |_| {
                                ...
                                spawn(async move {
                                    ...
                                    match worktrees.remove(&workspace, &target_id, false).await {
                                        Ok(()) => {
                                            files_session.reset();
                                            remove_target.set(None);
                                            menu_open.set(false);
