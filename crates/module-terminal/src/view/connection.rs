//! Shared terminal transport lifecycle, protocol handshake, and server-message reduction.
...
pub(super) fn use_terminal_connection(
    ...
    use_coroutine(move |commands| {
        run_connection(ports.clone(), options.clone(), state.clone(), commands)
    })
}
...
async fn run_connection(
    ...
    'connections: loop {
        ...
        let Some(transport) = ports.transport().cloned() else {
            connection.set(ConnectionState::Failed(
                "Interactive terminal sessions are unavailable in this runtime".into(),
            ));
            return;
        };
        let connect = transport.connect(&workspace).fuse();
        let connect_timeout =
        ...
        loop {
            ...
            let incoming = socket.receive().fuse();
