const colonLocationPattern =
  /(?:^|[\s([{"'])(?:--?>\s*)?((?:\.{1,2}\/)?(?:[A-Za-z0-9_@.+-]+\/)*[A-Za-z0-9_@.+-]+\.[A-Za-z0-9_-]+):(\d+)(?::(\d+))?(?:-(?:(\d+):)?(\d+))?/g;
const parenthesizedLocationPattern =
  /(?:^|[\s[{"'])(?:--?>\s*)?((?:\.{1,2}\/)?(?:[A-Za-z0-9_@.+-]+\/)*[A-Za-z0-9_@.+-]+\.[A-Za-z0-9_-]+)\((\d+),(\d+)\)/g;
...
function collectMatches(text, pattern, parenthesized) {
  ...
  return Array.from(text.matchAll(pattern), (match) => {
    ...
    const explicitEndLine = !parenthesized && match[4] ? Number.parseInt(match[4], 10) : null;
    const endColumn = !parenthesized && match[5] ? Number.parseInt(match[5], 10) : null;
    return {
