...
#[async_trait(?Send)]
pub trait MainTerminalCommandsTransport: Clone + Send + Sync + 'static {
...
#[derive(Clone)]
pub struct MainTerminalCommands<T> {
    transport: T,
}
...
where
    T: MainTerminalCommandsTransport,
{
    TerminalPorts::default()
        .with_transport(transport)
        .with_commands(PortHandle::new(MainTerminalCommands {
            transport: commands.clone(),
        }))
        .with_command_mutations(PortHandle::new(MainTerminalCommands {
            transport: commands,
        }))
...
#[async_trait(?Send)]
impl<T: MainTerminalCommandsTransport> TerminalCommandsPort for MainTerminalCommands<T> {
...
#[async_trait(?Send)]
impl<T: MainTerminalCommandsTransport> TerminalCommandMutationsPort for MainTerminalCommands<T> {
    async fn add(
        &self,
