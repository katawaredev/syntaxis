#![allow(
    ...
    reason = "Preview ports use cross-runtime PortHandle values and async UI callbacks"
)]

use crate::{
    PreviewCandidate, PreviewConfig, PreviewLease, PreviewPorts, PreviewShare, PreviewTarget,
};
use dioxus::prelude::*;
...
#[component]
pub(crate) fn PreviewLoading() -> Element {
    rsx! {
        div {
...
#[component]
pub(crate) fn ProcessPreviewView(workspace: WorkspaceRecord) -> Element {
    let ports = use_context::<PreviewPorts>();
    let clipboard = use_context::<FilesPorts>();
    let preview = ports
        .preview()
        .cloned()
        .expect("PreviewView requires a preview port");
    let preview_config = ports
        .config()
        .cloned()
        .expect("Process preview requires a configuration port");
    let process = ports
        .process()
        .cloned()
        .expect("Process preview requires a process port");
    let can_share = ports.share().is_some();
    let config_workspace = workspace.clone();
    let config_preview = preview_config.clone();
    let config = use_resource(move || {
        let workspace = config_workspace.clone();
        let preview = config_preview.clone();
        async move { preview.load(&workspace).await }
    });
    let candidates_workspace = workspace.clone();
    let candidates_preview = preview.clone();
    let mut candidates = use_resource(move || {
        let workspace = candidates_workspace.clone();
        let preview = candidates_preview.clone();
        async move { preview.candidates(&workspace).await }
    });
    let commands_workspace = workspace.clone();
    let commands_process = process.clone();
    let commands = use_resource(move || {
        let workspace = commands_workspace.clone();
        let process = commands_process.clone();
        async move { process.commands(&workspace).await }
    });
    let process_workspace = workspace.clone();
    let status_process = process.clone();
    let process_status = use_resource(move || {
        let workspace = process_workspace.clone();
        let process = status_process.clone();
        async move { process.status(&workspace).await }
    });
    let session_workspace = workspace.clone();
    let session_preview = preview.clone();
    let session = use_resource(move || {
        let workspace = session_workspace.clone();
        let preview = session_preview.clone();
        async move { preview.resume(&workspace).await }
    });
        ...
        match result {
            Ok(config) => {
                ...
                match target {
                    ...
                    Some(PreviewTarget::StaticDocument { .. }) => {
                        port.set("5173".into());
                    }
            ...
            Err(problem) => {
                port.set("5173".into());
                set_preview_error(toast, problem.to_string());
            }
        }
    });
    use_effect(move || {
        ...
        match result {
            ...
            Err(problem) => set_preview_error(toast, problem.to_string()),
        }
    });
    use_effect(move || {
    ...
    let connect_workspace = workspace.clone();
    let connect_preview = preview.clone();
    let connect = move || {
        ...
        connect_preview_target(
            connect_workspace.clone(),
            connect_preview.clone(),
            target,
            connection_state,
        );
    };
    let auto_workspace = workspace.clone();
    let auto_preview = preview.clone();
    use_effect(move || {
    ...
    let share_workspace = workspace.clone();
    let share_port = ports.share().cloned();
    let mut enable_sharing = move || {
        ...
        sharing.set(true);
        toast.set(None);
        let workspace = share_workspace.clone();
        let share_port = share_port.clone();
        spawn(async move {
            let Some(share_port) = share_port else {
                set_preview_error(toast, "Sharing is unavailable in this runtime.");
                sharing.set(false);
                return;
            };
            match share_port.create(&workspace, &active_lease).await {
    ...
    let revoke_port = ports.share().cloned();
    let mut revoke_sharing = move || {
        ...
        sharing.set(true);
        toast.set(None);
        let workspace = revoke_workspace.clone();
        let revoke_port = revoke_port.clone();
        spawn(async move {
            let Some(revoke_port) = revoke_port else {
                set_preview_error(toast, "Sharing is unavailable in this runtime.");
                sharing.set(false);
                return;
            };
            match revoke_port.revoke(&workspace, &active_lease).await {
                Ok(()) => share.set(None),
                Err(problem) => set_preview_error(toast, problem.to_string()),
            }
            sharing.set(false);
    ...
    let manage_workspace = workspace.clone();
    let manage_process = process.clone();
    let toggle_process = move || {
        ...
        toast.set(None);
        let workspace = manage_workspace.clone();
        let process = manage_process.clone();
        ...
        spawn(async move {
            ...
            let result = if stopping {
                process.stop(&workspace, &stop).await
            } else {
                process.start(&workspace, &start, &stop).await
            };
            match result {
                Ok(status) => {
                    ...
                    if status.running {
                        ...
                        if let Ok(status) = process.status(&status_workspace).await
                            && !status.running
                        {
    ...
    let save_settings_workspace = workspace.clone();
    let save_settings_preview = preview_config;
    let persist_settings = move || {
        ...
        process_busy.set(true);
        toast.set(None);
        let workspace = save_settings_workspace.clone();
        let preview = save_settings_preview.clone();
        let start = start_command().trim().to_owned();
        let stop = stop_command().trim().to_owned();
        ...
        spawn(async move {
            ...
            match preview.save(&workspace, config).await {
                Ok(()) => {
                ...
                InteractivePopover {
                    ...
                    if let Some(active_lease) = lease() {
                        div { class: "mt-3 grid gap-1 border-t border-border pt-2",
                            ...
                            a {
                                class: "flex h-8 items-center gap-2 rounded-md px-2 text-left text-xs text-muted-foreground hover:bg-accent hover:text-foreground",
                                href: active_lease.url.clone(),
                                target: "_blank",
                                rel: "noopener noreferrer",
                                Icon { icon: AppIcon::ExternalLink, size: 14 }
                                "Open in new tab"
                            }
                            if can_share && share().is_none() {
                                button {
                ...
                div { class: "flex min-h-10 flex-wrap items-center gap-2 border-b border-amber-500/25 bg-amber-500/8 px-3 py-1.5 text-xs",
                    ...
                    button {
                        ...
                        onclick: {
                            ...
                            let files = clipboard.clone();
                            move |_| copy_preview_link(url.clone(), files.clone(), toast)
                        },
...
fn connect_preview_target(
    ...
    preview: syntaxis_app_contracts::PortHandle<dyn crate::PreviewPort>,
    target: PreviewTarget,
    ...
    spawn(async move {
        match preview.open(&workspace, target).await {
            Ok(created) => {
                state.lease.set(Some(created));
                state.share.set(None);
                *state.reload_key.write() += 1;
            }
            Err(problem) => set_preview_error(state.toast, problem.to_string()),
        }
...
fn copy_preview_link(value: String, files: FilesPorts, mut toast: Signal<Option<(String, Tone)>>) {
    let clipboard = files.clipboard().cloned();
    spawn(async move {
        let Some(clipboard) = clipboard else {
            set_preview_error(toast, "Clipboard access is unavailable.");
            return;
        };
        match clipboard.copy_text(&value).await {
