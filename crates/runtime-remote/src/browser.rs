...
#[derive(Clone, Copy, Debug, Default)]
pub struct ClientImagePreview;

impl ImagePreviewPort for ClientImagePreview {
    fn create(&self, mime: &str, content: Vec<u8>) -> Result<ImageSource, AppError> {
        #[cfg(target_arch = "wasm32")]
        {
            ...
            return Ok(ImageSource::new(
                url,
                Some(PortHandle::new(ClientImageCleanup)),
            ));
        }
        #[cfg(not(target_arch = "wasm32"))]
        Ok(ImageSource::new(
...
#[derive(Clone, Copy, Debug, Default)]
#[cfg(target_arch = "wasm32")]
struct ClientImageCleanup;

#[cfg(target_arch = "wasm32")]
impl ImageSourceCleanup for ClientImageCleanup {
