...
impl PreferenceStore {
    fn open(path: PathBuf) -> Result<Self, String> {
        let file = match fs::read(&path) {
            ...
            Err(error) if error.kind() == std::io::ErrorKind::NotFound => PreferenceFile::default(),
    ...
    fn preferences_mut(&mut self, workspace_id: String) -> &mut ModelPreferences {
...
pub(super) fn sync(
    ...
    let mut store = store()?;
    Ok(store.preferences_mut(workspace_id.0).clone())
}
...
pub(super) fn set_favourite(
    ...
    let mut store = store()?;
    let preferences = store.preferences_mut(workspace_id.0);
    preferences
        .favourites
...
mod tests {
    ...
    #[test]
    fn unsupported_flat_preferences_are_not_migrated_to_an_arbitrary_workspace() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("model-preferences.json");
        fs::write(&path, r#"{"favourites":["p\u001fmodel"],"efforts":{}}"#).unwrap();
