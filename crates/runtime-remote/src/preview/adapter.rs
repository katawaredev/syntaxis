...
#[async_trait(?Send)]
pub trait RemotePreviewTransport: Clone + Send + Sync + 'static {
...
#[derive(Clone)]
pub struct RemotePreviewAdapter<T> {
    transport: T,
}

pub fn preview_ports<T>(transport: T) -> PreviewPorts
where
    T: RemotePreviewTransport,
{
    let adapter = PortHandle::new(RemotePreviewAdapter { transport });
...
#[async_trait(?Send)]
impl<T: RemotePreviewTransport> PreviewPort for RemotePreviewAdapter<T> {
    async fn candidates(
        &self,
...
#[async_trait(?Send)]
impl<T: RemotePreviewTransport> PreviewConfigPort for RemotePreviewAdapter<T> {
...
#[async_trait(?Send)]
impl<T: RemotePreviewTransport> PreviewProcessPort for RemotePreviewAdapter<T> {
...
#[async_trait(?Send)]
impl<T: RemotePreviewTransport> PreviewSharePort for RemotePreviewAdapter<T> {
    async fn create(
        &self,
        workspace: &WorkspaceRecord,
        lease: &PreviewLease,
