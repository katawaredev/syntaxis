...
pub(super) async fn create_preview_lease(
    ...
    let upstream = match &target {
        ...
        PreviewTarget::StaticDocument { .. } => {
            return Err(request_error(
                "Static document previews are available only in the browser runtime.",
                400,
            ));
        }
    };
    ...
    let saved_target = match target {
        ...
        PreviewTarget::StaticDocument { .. } => unreachable!("static targets were rejected"),
    };
    ...
    Ok(PreviewLease::persistent(lease_id, public_url.into()))
}
...
fn refresh_preview_session(
    ...
    Ok(PreviewSession {
        lease: PreviewLease::persistent(lease_id.to_owned(), url),
