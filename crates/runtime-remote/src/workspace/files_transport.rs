use crate::{RemoteFilesTransport, app_error_from_workspace};
...
use syntaxis_module_files::{
    FileGitPort, LanguageServiceConnection, LanguageServicesPort, SearchRequest, SearchResults,
};
#[cfg(feature = "server")]
...
pub(crate) fn runtime_services() -> syntaxis_app_shell::AppServices {
    let services =
        syntaxis_app_shell::AppServices::new(syntaxis_app_contracts::WorkspaceEventBus::default());
    #[cfg(feature = "host")]
    let services = services;
    #[cfg(not(feature = "host"))]
    let services = services.with_files(crate::remote_files_ports(DioxusFilesTransport));
    let files = services
        .files()
        .cloned()
    ...
    services.with_files(
        files
            ...
            .with_clipboard(PortHandle::new(crate::BrowserClipboard))
            .with_image_preview(PortHandle::new(crate::ClientImagePreview)),
    )
}
