#![allow(
    ...
    reason = "Runtime registration fans one adapter into the Git capability ports"
)]
...
#[async_trait(?Send)]
impl GitConflictPort for DioxusGit {
...
impl GitNetworkPort for DioxusGit {
    ...
    async fn update(
        ...
        api::update_remote(workspace_key(workspace), previous_name.to_owned(), request)
            .await
            .map_err(map_error)
    }
    ...
    async fn fetch(&self, workspace: &WorkspaceRecord) -> Result<RemoteResult, AppError> {
        api::fetch(workspace_key(workspace))
            .await
            .map_err(map_error)
    }

    async fn pull(&self, workspace: &WorkspaceRecord) -> Result<RemoteResult, AppError> {
        let result = api::pull(workspace_key(workspace))
            .await
            .map_err(map_error)?;
        self.changed(workspace);
        Ok(result)
...
impl GitWorktreePort for DioxusGit {
    async fn list(&self, workspace: &WorkspaceRecord) -> Result<Vec<WorktreeInfo>, AppError> {
        api::worktrees(workspace_key(workspace))
            .await
            .map_err(map_error)
    }
