...
package. It is bundled with esbuild into
`crates/runtime-remote/assets/terminal.bundle.js` (browser IIFE format, minified)
by the build script. The xterm.js CSS is inlined into the bundle and injected
into the document on first mount.
...
This generates the runtime-remote asset. The generated file is git-ignored — run
the build after `bun install`. The build script stores a content hash beside the
bundle and skips esbuild while its source files and pinned package versions are
unchanged. `runtime-remote` loads the versioned bridge idempotently when a terminal
renderer is mounted; shared terminal UI does not own the script lifecycle.
