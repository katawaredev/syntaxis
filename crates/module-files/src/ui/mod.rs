//! Canonical Files/Editor component.
...
use syntaxis_editor::{
    BufferStatus, EditorConfigSource, ExplorerTree, ExternalChange, IndentStyle, find_matches,
    language_label_for_path, language_servers_for_language, language_slug_for_path,
...
use syntaxis_workspace::{
    ChangeKind, EntryKind, FileEntry, FileSession, RelativePath, WorkspaceRecord,
};
...
use self::workspace::load_initial;
...
use documents::{
    open_document, reconcile_workspace_change, reload_document, restore_documents, save_all,
...
use git_actions::{
    GitDiscardContext, discard_git_change, run_file_action, show_diff, toggle_diff, toggle_stage,
};
...
pub fn FilesView(
    ...
    query: FilesQuery,
    on_navigate: EventHandler<FilesQuery>,
) -> Element {
    match workspace {
        Some(workspace) => rsx! {
            WorkspaceFiles {
...
fn WorkspaceFiles(
    ...
    let change_directory_files = files.clone();
    use_effect(move || {
    ...
    let language_services = files.language_services().cloned();
    let clipboard_available = files.clipboard().is_some();
    let language_services_available = language_services.is_some();
    ...
    use_effect(move || {
        ...
        let Some(language_services) = language_services.clone() else {
            language_service_connections.set(Vec::new());
            language_service_request.set(None);
            language_service_states.set(Vec::new());
            return;
        ...
        spawn(async move {
            for (server_id, server_name) in requested_servers {
                match language_services.open(&workspace, &server_id).await {
                    Ok(connection) => {
                        if language_service_request().as_deref() != Some(&request_key) {
                            return;
                        }
    ...
    use_effect(move || {
        ...
        explorer::load_change_directories(
            &change_directory_files,
            pending,
            workspace(),
            tree,
            editor_configs,
    ...
    let diff_files = files.clone();
    let stage_files = files.clone();
    let explorer_open_files = files.clone();
    ...
    let explorer_search_files = files.clone();
    let explorer_search_open = EventHandler::new(move |result: WorkspaceSearchResult| {
        ...
        let matches = result
            ...
            .collect();
        explorer_highlights.set(Some((path.clone(), matches)));
        pending_search_navigation.set(result.target.map(|target| {
            (
                path,
                EditorRange {
                    start: target.start,
    ...
    let explorer_files = files.clone();
    let explorer_expand = EventHandler::new(move |entry| {
        expand_directory(
            explorer_files.clone(),
            entry,
            workspace(),
            tree,
            editor_configs,
                    ...
                    div { class: "flex items-center gap-1",
                        ...
                        DropdownMenu {
                            ...
                            MenuContent { class: "right-0 w-60",
                                ...
                                EditorMenuItem {
                                    ...
                                    disabled: !language_services_available,
                                ...
                                EditorMenuItem {
                                    ...
                                    onclick: {
                                        ...
                                        move |()| save_all(&files, workspace().as_ref(), documents, toast)
                                    },
                                }
                                EditorMenuItem {
                                    index: 13,
                                ...
                                EditorMenuItem {
                                    ...
                                    onclick: move |()| toggle_stage(stage_files.clone(), workspace(), stage_change.clone(), refresh, toast),
                                }
                                hr {}
                                EditorMenuItem {
                                    index: 17,
                                    ...
                                    onclick: move |()| {
                                        match active_revert_action {
                                            Some(RevertAction::Unsaved) => revert_text_document(active_path(), documents),
                        ...
                        IconButton {
                            ...
                            onclick: {
                                ...
                                move |_| {
                                if let Some(path) = active_path() {
                                    save_path(files.clone(), workspace(), path, documents, toast);
                                }
                            }
                            },
                        }
...
async fn upload_files(
    ...
    let destination = match RelativePath::try_from(destination) {
