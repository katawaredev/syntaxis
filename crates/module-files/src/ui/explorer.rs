...
use super::{
    ...
    MapExtension, MetaExtension, MeterExtension, MpaddedExtension, MspaceExtension,
...
pub(super) fn Explorer(
    ...
    let files = use_context::<FilesPorts>();
...
pub(super) fn expand_directory(
    ...
    expand_loaded_directory(files, entry, workspace, tree, editor_configs, toast);
}
pub(super) fn reload_loaded_directories(
    ...
    for path in directories {
        ...
        expand_loaded_directory(
            FilesPorts::clone(files),
            entry,
            workspace.clone(),
            tree,
            editor_configs,
...
fn expand_loaded_directory(
    ...
    spawn(async move {
        match load_files_directory(&files, &workspace, entry.path).await {
            Ok(loaded) => {
                let directory = loaded.path.as_str();
                if let Some(source) = loaded.editor_config {
                    let mut configs = editor_configs.write();
                    if let Some(current) = configs
                        .iter_mut()
                        .find(|current| current.directory == directory)
                    {
                        *current = source;
                    } else {
                        configs.push(source);
                    ...
                    editor_configs
                        .write()
                        .retain(|current| current.directory != directory);
                }
                tree.write().replace_directory(directory, loaded.entries);
            }
