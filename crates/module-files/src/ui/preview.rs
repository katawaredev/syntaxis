...
pub fn render_markdown(source: &str) -> String {
    render_markdown_with_breaks(source, false)
}

pub fn render_markdown_preserving_newlines(source: &str) -> String {
    render_markdown_with_breaks(source, true)
}

fn render_markdown_with_breaks(source: &str, preserve_newlines: bool) -> String {
    ...
    let events = Parser::new_ext(source, options).map(|event| match event {
        ...
        Event::SoftBreak if preserve_newlines => Event::HardBreak,
        event => event,
    });
...
#[cfg(test)]
mod tests {
    use super::{parse_csv, render_markdown, render_markdown_preserving_newlines};
    ...
    fn preserves_newlines_when_requested() {
        let rendered = render_markdown_preserving_newlines("first line\nsecond line");

        assert!(rendered.contains("first line<br />\nsecond line"));
    }
