//! Session usage presentation shared by all conversation services.
...
#[component]
pub(crate) fn UsageMenu(
    stats: Option<AiUsage>,
        ...
        InteractivePopover {
                ...
                span { class: "relative grid size-6 place-items-center",
                    svg {
                        ...
                        circle {
                            class: "stroke-muted",
                            cx: "12", cy: "12", r: "9",
                            path_length: "100", stroke_width: "3",
                        }
                        circle {
            ...
            if let Some(stats) = stats {
                ...
                dl { class: "mt-2 grid grid-cols-2 gap-1.5 text-[10px]",
                    UsageStat { label: "Session tokens", value: compact_number(stats.total_tokens) }
                    UsageStat { label: "Estimated cost", value: format_cost(stats.cost_microusd) }
                    UsageStat { label: "Messages", value: stats.total_messages.to_string() }
...
#[component]
fn ContextUsage(stats: AiUsage) -> Element {
    ...
    rsx! {
        div { class: "rounded-lg border border-border bg-background/60 p-2.5",
            ...
            div { class: "mt-2 h-1.5 overflow-hidden rounded-full bg-muted",
                div { class: usage_bar_class(percent), style: "width: {percent}%" }
            }
...
mod tests {
    ...
    #[test]
    fn context_percent_uses_reported_value_then_tokens_and_clamps() {
        let mut usage = AiUsage {
            context_tokens: Some(80),
            context_window: Some(100),
            ..AiUsage::default()
        };
        assert_eq!(context_percent(&usage), 80);
        usage.context_percent = Some(65);
        assert_eq!(context_percent(&usage), 65);
        usage.context_percent = Some(255);
        assert_eq!(context_percent(&usage), 100);
        usage.context_percent = None;
        usage.context_window = Some(0);
        assert_eq!(context_percent(&usage), 100);
        usage.context_tokens = Some(u64::MAX);
        assert_eq!(context_percent(&usage), 100);
        usage.context_window = Some(u64::MAX);
        assert_eq!(context_percent(&usage), 100);
        assert_eq!(context_percent(&AiUsage::default()), 0);
    }

    #[test]
    fn context_severity_matches_the_ring_and_bar() {
        for (percent, severity) in [(0, "primary"), (65, "warning"), (85, "destructive")] {
            assert!(usage_ring_class(percent).ends_with(severity));
            assert!(usage_bar_class(percent).ends_with(severity));
        }
    }
}
