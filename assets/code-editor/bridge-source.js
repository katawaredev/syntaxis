...
(async () => {
  ...
  const highlightStyle = HighlightStyle.define([
    ...
    { tag: [tags.keyword, tags.modifier], color: "var(--dxc-light-a-k-color, #bb9af7)" },
    { tag: [tags.string, tags.regexp], color: "var(--dxc-light-a-s-color, #9ece6a)" },
    { tag: [tags.number, tags.bool, tags.null], color: "var(--dxc-light-a-l-color, #ff9e64)" },
    { tag: [tags.function(tags.variableName), tags.function(tags.propertyName)], color: "var(--dxc-light-a-f-color, #7aa2f7)" },
    { tag: [tags.typeName, tags.className, tags.namespace], color: "var(--dxc-light-a-t-color, #2ac3de)" },
    { tag: [tags.propertyName, tags.attributeName], color: "var(--dxc-light-a-v-color, #c0caf5)" },
    { tag: [tags.operator, tags.punctuation], color: "var(--dxc-light-a-o-color, #89ddff)" },
    { tag: [tags.tagName, tags.heading], color: "var(--dxc-light-a-td-color, #f7768e)" },
    { tag: [tags.link, tags.url], color: "var(--dxc-light-a-tu-color, #7aa2f7)", textDecoration: "underline" },
  ...
  view = new EditorView({
    ...
    state: EditorState.create({
      ...
      extensions: [
        ...
        wrappingCompartment.of(initial.word_wrap ? EditorView.lineWrapping : []),
        tabSizeCompartment.of(EditorState.tabSize.of(Math.max(1, initial.tab_width))),
        indentCompartment.of([indentUnit.of(indentText(initial)), indentOnInput()]),
        readOnlyCompartment.of(
          EditorState.readOnly.of(initial.read_only || isDiff(initial)),
        ),
        attributesCompartment.of(EditorView.contentAttributes.of(editorAttributes(initial))),
  ...
  const configure = config => {
    ...
    const effects = [
      ...
      wrappingCompartment.reconfigure(config.word_wrap ? EditorView.lineWrapping : []),
      autocompleteCompartment.reconfigure(
        config.autocomplete && !isDiff(config) ? autocompletion() : [],
      ),
      tabSizeCompartment.reconfigure(EditorState.tabSize.of(Math.max(1, config.tab_width))),
      indentCompartment.reconfigure([indentUnit.of(indentText(config)), indentOnInput()]),
      readOnlyCompartment.reconfigure(
        EditorState.readOnly.of(config.read_only || isDiff(config)),
      ),
      attributesCompartment.reconfigure(EditorView.contentAttributes.of(editorAttributes(config))),
      placeholderCompartment.reconfigure(config.placeholder ? placeholder(config.placeholder) : []),
      setSearchRanges.of(
        searchDecorations(config.value, config.search_matches, config.active_search_match),
      ),
    ];
    if (diffChanged) {
