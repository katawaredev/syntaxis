...
export async function startEditor(dioxus) {
  ...
  const codeUnitToByte = (value, offset) => encoder.encode(value.slice(0, offset)).length;
  ...
  const indentText = (config) =>
    config.indent_with_tabs ? "\t" : " ".repeat(Math.max(1, config.indent_width));
  ...
  const rebuildSearch = (state) => {
    const query = getSearchQuery(state);
    editorSearchMatches =
      currentSearchQuery && query.valid && query.search ? Array.from(query.getCursor(state)) : [];
    emitSearch(state);
  };
  ...
  view = new EditorView({
    ...
    state: EditorState.create({
      ...
      extensions: [
        ...
        readOnlyCompartment.of(EditorState.readOnly.of(initial.read_only || isDiff(initial))),
  ...
  const configure = (config) => {
    ...
    const languageChanged = config.language !== currentConfig.language || filenameChanged;
    ...
    const effects = [
      ...
      readOnlyCompartment.reconfigure(EditorState.readOnly.of(config.read_only || isDiff(config))),
