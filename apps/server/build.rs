...
fn main() {
    ...
    fs::create_dir_all(&app_assets).expect("failed to create the server app asset directory");
    ...
    for relative_path in COPIED_ASSETS {
        ...
        if let Some(parent) = destination.parent() {
            fs::create_dir_all(parent).expect("failed to create a server app asset subdirectory");
        }
        fs::copy(&source, &destination).unwrap_or_else(|error| {
            panic!(
                "failed to stage server app asset {}: {error}",
                source.display()
            )
        });
    }
