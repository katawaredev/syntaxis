use dioxus::prelude::*;
use syntaxis_app_shell::use_active_workspace;
use syntaxis_module_files::{use_files_controller, use_files_ui_state};
...
use super::{WorkspaceListCache, events::WorkspaceEventBridge};
use crate::ai::notifications::NotificationMenu;

#[component]
pub fn WorkspaceShell() -> Element {
    let files_session = use_files_controller();
    use_context_provider(|| files_session);
    let files_ui = use_files_ui_state();
    use_context_provider(|| files_ui);
    use_effect(move || {
        let _ = files_ui.reset_revision();
        files_session.reset();
    });
