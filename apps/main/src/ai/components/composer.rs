...
pub(crate) fn AgentComposer(
    ...
    let mention_bootstrap = use_resource(move || {
        ...
        let files = bootstrap_files.clone();
        async move {
            let entries = files
                .files()
            ...
            let ignored_paths = if let Some(git) = files.git() {
            ...
            Ok::<_, String>((entries, ignored_paths))
        }
    });
    ...
    let mention_results = use_resource(move || {
        ...
        let root_candidates = bootstrap
            ...
            .unwrap_or_default();
        let ignored_paths = bootstrap.map(|(_, ignored_paths)| ignored_paths);
        async move {
